...
constructor(props) {
  ...
  componentDidMount() {
      const [x1, y1] = [Math.random()*this.state.width, Math.random()*this.state.height];
      const [x2, y2] = [Math.random()*this.state.width, Math.random()*this.state.height];
      const [id1, id2] = [uuid(16,16), uuid(16,16)]
      this.setState({
          items: [{
            "id"                    : id1,
        ...
        connectRalation: [{
            x1                  : x1+this.state.tableWidth/2,
            y1                  : y1+this.state.tableHeight/2,
            x2                  : x2+this.state.tableWidth/2,
            y2                  : y2+this.state.tableHeight/2,
            startId             : id1,
            startCircleIndex    : 0,
            endId               : id2,
            endCircleIndex      : 1
        ...
        renderLines({
            ...
            relation    : this.state.connectRalation
        });
      })
      let $this = this;
      /* 注册resize */
      ...
      /*** 绘制箭头 ***/    
        ...
        function svgZoomed(d) {
            $outerRegion.attr("transform", d.transform);
        }
  ...
  updateTableList(){
      if((this.state.items.length > 0)){
          ...
          let $notFixedItem = this.state.items.filter(item => item.fixed !== true);
          if($notFixedItem.length > 0){
              this.outerTableRegion = this.outerRegion.selectAll("g.tableRegion")
                  .data($notFixedItem)
                  .enter()
                  .append("g")
                  .attr("data-id",d => d.id)
              ...
              this.outerTableRegion.append("rect")
          ...
          if($fixedItem.length > 0){
              this.outerFixedTableRegion = this.outerFixRegion.selectAll("g.tableFixedRegion")
                  .data($fixedItem)
                  .enter()
                  .append("g")
                  .attr("data-id",d => d.id)
              ...
              this.outerFixedTableRegion
                  ...
                  .attr("d",d => {
                      ...
                      if($fixedItem.length > 1 && d.index === 0){
                          $str = `M${x1},${y1} ${x2},${y2} ${x2},${y2+(--$fixedItem.length*($tableHeight+30)+1)}`;
                      }else{
                          $str = `M${x1},${y1} ${x2},${y2}`;
                      }
                      return $str;
  ...
  tableStarted(d){
      console.log(d)
      if(typeof d === "object"){
          d.dx = d.sourceEvent.x;
          d.dy = d.sourceEvent.y;
      }
  }
  tableDragIng(d){
      if(typeof d === "object"){
          d.xp = d.x - (d.dx - d.sourceEvent.x);
          d.yp = d.y - (d.dy - d.sourceEvent.y);
          d3.select(this).attr("transform", (d) => `translate(${d.xp}, ${d.yp})`).attr("data-x",d.xp).attr("data-y",d.yp);
          ...
          if($start){
              for (let [key,element] of $start.entries()) {
                  ...
                  element.setAttribute("d",`M${$x},${$y} ${$element.x2},${$element.y2}`);
          ...
          if($end){
              for (let [key,element] of $end.entries()) {
                  ...
                  element.setAttribute("d",`M${$element.x1},${$element.y1} ${$x},${$y}`);
  ...
  fixedTableDragIng(d){
      ...
      d.yp = d.y - (d.dy - d.sourceEvent.y);
      d3.select($this)
          .attr("transform", (d) => `translate(${d.xp}, ${d.yp})`).attr("data-x",d.xp).attr("data-y",d.yp);
  }
  fixedTableDragEnd(d){
      d.x = d.xp;
      d.y = d.yp;
  ...
  sortByClick(index){
      if(this.state.items.length > 1){
          ...
          timer = setTimeout(() => {
              ...
              let $arr = this.state.items.concat($del).map(function(obj, $index, arr) {
                  obj.index = $index;
                  return obj;
              });
              this.setState(state => ({
...
function renderSmallNode (params){
  ...
  function pathDragIng(d){
      if(typeof d === "object"){
          ...
          const x1    = d.x+xDis;
          const y1    = d.y+yDis;
          const x2    = d.sourceEvent.x + xDis;
          const y2    = d.sourceEvent.y + yDis;
